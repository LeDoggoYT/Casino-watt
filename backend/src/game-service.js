...
function settleRound(round, outcome) {
  ...
  const roundBalanceAfter = balanceRow.amount + payout;
  const netResult = roundBalanceAfter - balanceBefore;
  // Spielgeld wird niemals endgültig aufgebraucht: Nach einer Verlustserie
  // erhält der Account beim Erreichen von 0 automatisch einen neuen Startstand.
  const balanceAfter = roundBalanceAfter <= 0 ? 1000 : roundBalanceAfter;
  const balanceRefilled = roundBalanceAfter <= 0;
  ...
  const result = {
    ...
    message: `${outcomeMessage(outcome, round.playerCards, round.dealerCards)}${
      balanceRefilled ? " Kontostand automatisch auf 1.000 Chips erneuert." : ""
    }`
  };
