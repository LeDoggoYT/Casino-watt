...
export function requireAuth(req, res, next) {
  ...
  const session = db.prepare(`
    SELECT s.id AS session_id, s.user_id, s.expires_at,
           u.username, u.is_suspended, u.suspension_reason, b.amount AS balance
    FROM sessions s
  ...
  if (session.is_suspended) {
    return fail(res, 403, "Dieses Spielerkonto wurde gesperrt.", {
      suspended: true,
      reason: session.suspension_reason || "Kein Grund angegeben."
    });
  }
